...
public class Report {
	...
	private String handleStepInvolvingScreenshot(String stepName,
			Status stepStatus) {
		String screenshotName = testParameters.getCurrentTestcase()+"_"+"Screenshot_"+Util.getCurrentFormattedTime(
		...
		if ((stepStatus.equals(Status.FAIL) && reportSettings
			...
			if(new File(screenshotPath).exists()) {
				int intCounter = 1;
				boolean blnFileNameFound = false;
				do {
					String strScreenshotPath = reportSettings.getReportPath()
							+ Util.getFileSeparator() + SCREENSHOTS
							+ Util.getFileSeparator();
					String strNewScreenshotName = screenshotName.replace(".png", "")+"_"+intCounter+".png";
					if(!new File(strScreenshotPath+strNewScreenshotName).exists()) {
						screenshotPath = reportSettings.getReportPath()
								+ Util.getFileSeparator() + SCREENSHOTS
								+ Util.getFileSeparator() + strNewScreenshotName;
						screenshotName = strNewScreenshotName;
						blnFileNameFound = true;
					} else {
						intCounter++;
					}
				} while (!blnFileNameFound); 
			}
	...
	public void updateTestLogPassNS(String stepName, String stepDescription,
            Status stepStatus) {
     ...
     if (stepStatus.ordinal() <= reportSettings.getLogLevel()) {
            ...
            for (int i = 0; i < reportTypes.size(); i++) {
                  reportTypes.get(i).updateTestLogPassNS(Integer.toString(stepNumber),
                                stepName, stepDescription, stepStatus);
                  /***** To Inetgrate with JIRA *****/
                  /*if(stepStatus==Status.FAIL)
                  RESTclient.defectLog(testParameters.getCurrentTestcase(), stepDescription,
                          reportSettings.getReportPath()
