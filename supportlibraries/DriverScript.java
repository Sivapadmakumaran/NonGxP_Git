...
public class DriverScript {
	...
	public static SeleniumTestParameters testParametersCustomised;*/
	...
	private void initializeWebDriver() {
		...
		case REMOTE:
			WebDriver remoteWebDriver = WebDriverFactory.getRemoteWebDriver(
					testParameters.getBrowser(),null,testParameters.getPlatform(),
					properties.getProperty("RemoteUrl"));
			driver = new CraftDriver(remoteWebDriver);
			driver.setTestParameters(testParameters);
			WaitPageLoad();
	...
	private synchronized void initializeDatatable() {
		...
		String strLockFileName = "InitializeDatatable";
		FileLockMechanism objFileLockMechanism = new FileLockMechanism(Long.valueOf(properties.getProperty("FileLockTimeOut")));
		FileLock objFileLock = objFileLockMechanism.SetLockOnFile(strLockFileName);
		if(objFileLock!=null) {
			String datatablePath = frameworkParameters.getRelativePath()
				+ Util.getFileSeparator() + "Datatables";
		...
		dataTable = new CraftDataTable(runTimeDatatablePath,datatablePath,
				testParameters.getCurrentScenario());
		...
		if (properties.getProperty("Approach")
			...
			objFileLockMechanism.ReleaseLockOnFile(objFileLock, strLockFileName);
		}
	}

	private void initializeTestScript() {
		driverUtil = new WebDriverUtil(driver,report,ALMFunctions);
		scriptHelper = new ScriptHelper(dataTable, report, driver, driverUtil,testParameters.getBrowser().getValue(),testParameters.getCurrentTestcase(),ALMFunctions,testParameters);
	...
	public long getExecutionTimeInMilliSecs() {
